...
def generate_crypto_wallet():
    ...
    seed = request.args.get("seed") or "".join(
...
def generate_transaction():
    ...
    try:
        ...
        from_address, outputs, utxo_inputs = (
            data["from_address"],
            data["outputs"],
            data["utxo_inputs"],
        )
        ...
        outputs = [
            {"address": output["address"], "value": to_satoshis(output["value"])}
            for output in outputs
        ]
        ...
        if total_tx_in < total_tx_out:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "You are trying to create a transaction that will output more than the UTXOs inputted.",
                    }
                ),
                400,
            )
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "error": "Invalid value (are your output amounts valid?)",
                }
            ),
            400,
        )
    ...
    try:
        ...
        if resp is not None:
            ...
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "Something went wrong broadcasting the transaction. Please try again.",
                    }
                ),
                500,
            )
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "error": "Please ensure your private key is correct.",
                }
            ),
            400,
        )
